...
class CanEditUserGroupsMiddleware
{
    /**
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if(is_null($userId) || is_null($isValidated)) {
            return response()->json(['hasErrors' => true, 'message' => 'Missing permission can_edit_user_groups'], 403);
        }
        $member = $this->findMember($userId);
        if(!is_null($member) && $member->Group->can_access_admincp && $member->Group->can_edit_groups) {
            return $next($request);
        }
        return response()->json(['hasErrors' => true, 'message' => 'Missing permission can_edit_user_groups'], 403);
    }
